...
def bookservice(request, service_type, service_id):
    ...
    if request.method == 'POST':
        ...
        if payment_status == 'created':
            # Create a booking instance with the service name included
            booking = Booking.objects.create(
                ...
                amount=total / 100,
                order_id=order_id,
                # Set the name based on the type of service being booked
                hotel=service if service_type == 'hotel' else None,
                package=service if service_type == 'package' else None,
                guide=service if service_type == 'guide' else None
            )
...
def bookings_view(request):
    bookings = Booking.objects.filter(user=request.user, payment_status='paid').select_related('hotel', 'package', 'guide')
